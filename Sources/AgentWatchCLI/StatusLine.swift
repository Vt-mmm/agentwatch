// `agentwatch statusline` — single-line output cho Claude Code statusLine.
//
// Cách wire: thêm vào ~/.claude/settings.json:
//   {
//     "statusLine": {
//       "type": "command",
//       "command": "agentwatch statusline"
//     }
//   }
//
...
struct StatusLineCommand {
    func run() {
        ...
        if let info {
            // Failure to save must not break the host statusline. No raw payload is stored.
            try? QuotaSnapshotStore.local.save(QuotaParser.claudeStatusLine(info))
        }
