// agentwatch CLI — terminal pet mascot + live token/cost dashboard.
//
// Subcommands:
//   agentwatch watch         live mode, refresh 2s, ASCII pet phản ánh state
//   agentwatch status        snapshot 1 lần rồi exit (CI/script friendly)
//   agentwatch report [day|week]   stats coaching kỳ vừa qua
//
// Mặc định scan ~/.claude/projects + Desktop sessions, tự pick session mới nhất
// đang active. Tái sử dụng AgentWatchCore (PetState, JsonlParser, …) — không
// duplicate logic.
...
func printHelp() {
    let txt = """
    agentwatch — terminal companion cho Claude Code sessions.

    Usage:
      agentwatch watch                 live dashboard với pet (Ctrl+C để thoát)
      agentwatch status                snapshot 1 lần
    ...
    Inject pet vào Claude Code CLI:
      ...
      hoặc edit ~/.claude/settings.json:
        "statusLine": { "type": "command", "command": "agentwatch statusline" }
...
struct SetupCommand {
    func run() {
        ...
        let binary = "/usr/local/bin/agentwatch"
        ...
        if !FileManager.default.fileExists(atPath: binary) {
            ...
            print("   Em sẽ dùng path absolute hiện tại của agentwatch.")
        }
        let cmd = FileManager.default.fileExists(atPath: binary)
            ? binary
            : (CommandLine.arguments.first ?? "agentwatch")
...
func scanDaySessions() -> [SessionSummary] {
    ...
    return SessionInventory.list(in: start..<end)
...
func render(pet: PetState, sessions: [SessionSummary],
    ...
    print(colorize("agentwatch", Ansi.bold) +
