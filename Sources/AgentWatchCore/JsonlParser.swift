...
// Mirrors the legacy Python parser's session aggregation rules.
...
public enum JsonlParser {
    ...
    private static func parseSessionData(at url: URL,
        ...
        guard let handle = try? FileHandle(forReadingFrom: url) else {
            stats.usageLedger.recordWarning("Source file unreadable; usage is unavailable.")
        ...
        stats.inputTokens = usage.input; stats.outputTokens = usage.output
        stats.cacheReadTokens = usage.cacheRead; stats.cacheWriteTokens = usage.cacheWrite
    ...
    private static func applyLine(_ data: Data,
        ...
        case "assistant":
            ...
            do {
                ...
                let requestModel = msg["model"] as? String ?? stats.model
                stats.usageLedger.upsert(UsageEntry(
                    id: UsageIdentity.key(agent: "claude", provider: "anthropic",
                                          id: msg["id"] as? String ?? raw["uuid"] as? String,
                                          raw: data, sessionID: stats.sessionId),
                    sessionID: stats.sessionId, agent: "claude", provider: "anthropic",
                    modelID: requestModel, timestamp: timestamp ?? .distantPast,
                    tokens: UsageTokens(input: UsageIdentity.count(usage["input_tokens"], required: true),
                                        output: UsageIdentity.count(usage["output_tokens"], required: true),
                                        cacheRead: intValue(usage["cache_read_input_tokens"]),
                                        cacheWrite: intValue(usage["cache_creation_input_tokens"]),
                                        cacheWrite1h: intValue(cache["ephemeral_1h_input_tokens"])),
    ...
    private static func applyAssistantContent(_ content: [[String: Any]],
        ...
        for block in content {
            ...
            case "tool_use":
                ...
                if !id.isEmpty, !stats.observedToolIDs.insert(id).inserted { continue }
                stats.toolCalls += 1
