...
public enum ProjectPath {
    public static var projectsDir: URL { URL(fileURLWithPath: AgentLogRoots.current.claudeProjects) }
    ...
    public static func sessionFiles(slug: String, root: URL = projectsDir) -> [URL] {
    ...
    public static func latestSession(for cwd: URL, root: URL = projectsDir) -> URL? {
        var current: URL? = cwd.standardizedFileURL
        while let dir = current {
            let candidates = sessionFiles(slug: slug(for: dir), root: root)
    ...
    public static func mostRecentSessionAcrossAllProjects(root: URL = projectsDir) -> URL? {
