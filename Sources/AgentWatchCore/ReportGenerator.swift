...
public enum ReportGenerator {
    ...
    public static func markdown(scope: ReportScope,
        ...
        var md = "# \(AgentWatchIdentity.productName.uppercased()) REPORT — AI Coding theo SDD/VSDD\n\n"
        md += "**Phạm vi:** \(scope.label)\n"
        md += "**Locale:** language `\(AgentWatchLocale.languageCode)` · region `\(AgentWatchLocale.regionCode)` · `\(AgentWatchLocale.identifier)`\n"
        md += "**Múi giờ:** \(ReportTime.timeZoneLabel)\n"
        ...
        md += "- Claude: total = input + output + cache read + cache write; cost ước tính từ bảng giá AgentWatch.\n"
    ...
    public static func html(scope: ReportScope,
        ...
        return """
        <!DOCTYPE html><html lang="\(AgentWatchLocale.htmlLanguageTag)"><head><meta charset="UTF-8">
        <title>\(AgentWatchIdentity.productName) Report — \(htmlEscape(scope.label))</title>
        <style>
        ...
        <h1>\(AgentWatchIdentity.productName) Report</h1>
        <div class="muted">\(htmlEscape(scope.label)) · \(AgentWatchLocale.identifier) · \(ReportTime.timeZoneLabel) · accounting usage-v2 · prices \(Pricing.versionLabel)</div>
        <div class="stats">
        ...
        <ul>
          ...
          <li>Session rows are scoped to the selected \(ReportTime.timeZoneLabel) report period. Partial rows carry an explicit warning.</li>
        </ul>
    ...
    private static func costProvenance(_ session: SessionSummary) -> String {
        ...
        case .agentEstimated:
            return "agent-reported estimate; not an invoice; coverage \(session.costCoverage.rawValue)"
        case .estimated:
            if let entries = session.usageEntries {
                let versions = Set(entries.map(\.pricingVersion)).sorted().joined(separator: ", ")
                return "per-request list-price equivalent @ \(versions); coverage \(session.costCoverage.rawValue); not an invoice"
            }
    ...
    private static func toolAuditHTML(for session: SessionSummary,
        ...
        <div class="table-scroll"><table class="tool-table"><colgroup><col style="width:145px"><col style="width:105px"><col style="width:34%"><col style="width:105px"><col></colgroup><thead><tr><th>Time (\(ReportTime.timeZoneLabel))</th><th>Tool</th><th>Action</th><th>Status</th><th>Result</th></tr></thead><tbody>\(rows)</tbody></table></div>
        </details>
        """
    }
    ...
    private static func reportRange(for scope: ReportScope) -> Range<Date> {
