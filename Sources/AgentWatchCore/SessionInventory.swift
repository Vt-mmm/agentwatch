...
public enum UsageCostBasis: String, Sendable, Codable, Equatable {
    ...
    case reported
    /// Cost reconstructed from observed tokens and AgentWatch's price table.
    case estimated
    case agentEstimated
    /// The source did not expose cost and no trustworthy estimate was available.
    case unavailable

    public var isEstimate: Bool { self == .estimated || self == .agentEstimated }

    public var label: String {
        switch self {
        case .reported:    return "reported by source"
        case .estimated:   return "list-price estimate"
        case .agentEstimated: return "agent-reported estimate"
...
public struct SessionSummary: Identifiable, Sendable, Equatable, Codable {
    ...
    /// Distinguishes provider-reported cost from an AgentWatch estimate.
    ...
    public let usageEntries: [UsageEntry]?

    public var costCoverage: CostCoverage {
        usageEntries.map { UsageLedger(entries: $0).costCoverage }
            ?? (costBasis == .unavailable ? .unavailable : .complete)
    }
    ...
    public init(id: String, sessionTitle: String? = nil,
                ...
                dataWarnings: [String] = [], usageEntries: [UsageEntry]? = nil) {
        self.id = id
...
public enum SessionInventory {
    ...
    public static func list(in range: Range<Date>) -> [SessionSummary] {
        ...
        return SessionAccounting.canonical(out).sorted { a, b in
    ...
    public static func aggregate(_ sessions: [SessionSummary]) -> InventoryAggregate {
        let sessions = SessionAccounting.canonical(sessions)
        var ledger = UsageLedger()
        for session in sessions { for entry in session.usageEntries ?? [] { ledger.upsert(entry) } }
        ...
        input = t.input; output = t.output; reasoning = t.reasoning; cr = t.cacheRead; cw = t.cacheWrite
        totalTokens = t.total
        estimatedCost = NSDecimalNumber(decimal: ledger.knownCostSubtotal).doubleValue
        cost = estimatedCost
        tools = sessions.reduce(0) { $0 + $1.toolCallCount }
        unavailableCostSessionCount = sessions.filter { $0.costCoverage != .complete }.count
        for s in sessions where s.usageEntries == nil {
            input  += s.inputTokens
    ...
    private static func listProjects(in range: Range<Date>,
                                     index: DesktopOriginIndex) -> [SessionSummary] {
        let projectsDir = AgentLogRoots.current.claudeProjects
        ...
        for slug in projectDirs {
            ...
            guard let files = fm.enumerator(at: projectURL, includingPropertiesForKeys: nil, options: [.skipsHiddenFiles]) else {
                continue
            }
    ...
    private static func listDesktopAgent(in range: Range<Date>) -> [SessionSummary] {
    ...
    private static func summarize(file: URL, slug: String, display: String,
                                  ...
                                  range: Range<Date>) -> SessionSummary? {
