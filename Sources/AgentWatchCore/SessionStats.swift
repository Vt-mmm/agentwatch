...
public struct SessionStats: Sendable, Equatable {
    ...
    public var hasUsageLedger = false
    var observedToolIDs: Set<String> = []
    ...
    public var cost: Double {
        if hasUsageLedger { return NSDecimalNumber(decimal: usageLedger.knownCostSubtotal).doubleValue }
    ...
    public var costBasis: UsageCostBasis {
        if hasUsageLedger { return usageLedger.costBasis }
        return Pricing.quote(forModelId: model) == nil ? .unavailable : .estimated
    }

    public var priceSourceLabel: String? {
        if hasUsageLedger { return "Per-request estimates @ \(Pricing.versionLabel); \(usageLedger.costCoverage.rawValue) cost coverage" }
        return Pricing.quote(forModelId: model)?.sourceLabel
    }
