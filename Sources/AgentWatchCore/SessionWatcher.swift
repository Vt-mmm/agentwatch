...
public final class SessionWatcher {
    ...
    public init(projectsRoot: URL = ProjectPath.projectsDir) { self.projectsRoot = projectsRoot }
    ...
    private func currentTargetSession() -> URL? {
        if followLatest {
            return ProjectPath.mostRecentSessionAcrossAllProjects(root: projectsRoot)
        }
        guard let pinned = pinnedFolder else { return nil }
        return ProjectPath.latestSession(for: pinned, root: projectsRoot)
    }
}
